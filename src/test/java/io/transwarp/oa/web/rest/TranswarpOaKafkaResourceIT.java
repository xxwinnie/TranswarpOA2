...
import io.transwarp.oa.config.KafkaProperties;
...
class TranswarpOaKafkaResourceIT {
    ...
    void setup() {
        ...
        TranswarpOaKafkaResource kafkaResource = new TranswarpOaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/transwarp-oa-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/transwarp-oa-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
